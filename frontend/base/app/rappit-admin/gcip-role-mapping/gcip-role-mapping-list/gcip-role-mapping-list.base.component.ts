...
export class GcipRoleMappingListBaseComponent {
  ...
  tableConfig: any = {
    ...
    "detailPage": {
      "sid": "",
      "name": "gci role mapping Detail",
      "url": "admin/gcipdetail"
    },
  ...
  selectedRows!: any;
  pageViewTitle: string = 'GCIP_Role_Mapping';
  ...
  onRefresh(fromDelete?: boolean, fromFilters?: boolean): void {
    ...
    if (this.tableSearchControls.get('domainname')?.getRawValue())
      params.search['domainname'] = this.tableSearchControls.get('domainname')?.getRawValue();
  ...
  onActivate() {
    if (this.selectedValues.length > AppConstants.threshold) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          const activateSubscription = this.gcipRoleMappingService.activate(requestedParams).subscribe((res: any) => {
            if (res?.message) {
  ...
  onDeactivate() {
    if (this.selectedValues.length > AppConstants.threshold) {
      ...
      this.confirmationService.confirm({
        ...
        icon: 'pi pi-info-circle',
        accept: () => {
          const deactivateSubscription = this.gcipRoleMappingService.deactivate(requestedParams).subscribe((res: any) => {
            if (res?.message) {
              this.showToastMessage({ severity: 'success', summary: '', detail: this.translateService.instant(res.message) });
            } else {
  ...
  onNew() {
    ...
    let property: Exclude<keyof GcipRoleMappingListBaseComponent, ''> = value;

    if (this.isChildPage && this[property]) {
      const methodName: any = "onNewChild";
      let action: Exclude<keyof GcipRoleMappingListBaseComponent, ''> = methodName;
  ...
  actionBarAction(btn: any) {
    ...
    if (btn.action === 'navigate_to_page' && btn.pageName?.url) {
      ...
      const onSave: Exclude<keyof GcipRoleMappingListBaseComponent, ' '> = methodName.split('_')[0];
    ...
    if (this.selectedValues.length > AppConstants.threshold) {
      ...
      if (this.selectedValues.length > 0) {
        ...
        this.confirmationService.confirm({
          ...
          icon: 'pi pi-info-circle',
          accept: () => {
            const deleteSubscription = this.gcipRoleMappingService.delete(requestedParams).subscribe((res: any) => {
  ...
  getGridConfig() {
    ...
    const gridConfigData: any = {
      ...
      ajaxUrl: GcipRoleMappingApiConstants.getDatatableData,
      select: true,
  ...
  onFilterSelectionClick(event: any = AppConstants.softDeleteFilterDefaultValue, onPageLoad: Boolean = false) {
    ...
    if (!onPageLoad) {
      this.gridConfig.ajaxUrl = GcipRoleMappingApiConstants.getDatatableData
  ...
  loadGridData() {
    ...
    if (environment.prototype && this.tableConfig.children?.length > 0) {
      gridSubscription = this.gcipRoleMappingService.getProtoTypingData().subscribe((data: any) => {
  ...
  setupCall() { //initial setup for provider mapping
    if (!environment.prototype) {
      this.gcipRoleMappingService.setupCall({}).subscribe((res: any) => {
        if (res) {
          this.loadGridData();
        }
      });
