...
import { GcipRoleMappingService } from '../gcip-role-mapping.service';
...
export class GcipRoleMappingDetailBaseComponent {
  ...
  pageViewTitle: string = 'GCIP_Role_Mapping_Detail';

  public gcipRoleMappingService = inject(GcipRoleMappingService);
  ...
  detailFormControls: UntypedFormGroup = new UntypedFormGroup({
    domainname: new UntypedFormControl('', [Validators.maxLength(253)]),
    systemCreated: new UntypedFormControl(false),
    rolename: new UntypedFormControl([])
  });

  get selectedRoleNameControl() {
    return this.detailFormControls.get('rolename') as UntypedFormControl;
  }

  onSave(isToastNotNeeded?: boolean, actionName?: string) {
    this.detailFormControls.get('domainname')?.setValue(this.detailFormControls.get('domainname')?.value.trim().toLowerCase());
    ...
    if (this.checkValidations() && this.validateDomain()) {
      let preFormattedData = this.formatFormDataBeforeSave();
      preFormattedData['rolename'] = preFormattedData['rolename'].join(',');
      ...
      if (environment?.prototype && this.appGlobalService.get('saveAndCloseClicked')) {
        ...
        const saveSubscription = this.gcipRoleMappingService[method](requestedObj, actionName).subscribe((res: any) => {
  ...
  validateDomain() {
    const newDomain = (this.detailFormControls.get('domainname')?.value || '').trim().toLowerCase();
    ...
    const isLabelTooLong = labelsForDomains.some((label: any) => label.length > this.domainLabelMaxLength); // 63 is the Label length limit

    if (isInvalid || isDomainTooLong || isLabelTooLong) {
      if (isInvalid) {
        this.showMessage({ severity: 'error', summary: 'Error', detail: 'Invalid Domain name' });
        return false;
      } else if (isDomainTooLong) {
        this.showMessage({ severity: 'error', summary: 'Error', detail: 'Invalid Domain length' });
        return false;
      } else if (isLabelTooLong) {
        this.showMessage({ severity: 'error', summary: 'Error', detail: 'Invalid Domain label length' });
        return false;
      } else{
        return true;
      }
  ...
  onAfterSave(res: any, data: any, method: string, isToastNotNeeded?: boolean) {
    ...
    if (this.data['rolename'] && typeof this.data['rolename'] != 'string') {
      this.data['rolename'] = this.data['rolename'].join(',');
    }
  ...
  actionBarAction(btn: any) {
    ...
    let action: Exclude<keyof GcipRoleMappingDetailBaseComponent, ' '> = methodName;
    ...
    if (btn.action === 'navigate_to_page' && btn.pageName?.url) {
      ...
      const onSave: Exclude<keyof GcipRoleMappingDetailBaseComponent, ' '> = methodName.split('_')[0];
  ...
  formatRawData() {
    ...
    if (typeof patchData?.rolename == 'string') {
      this.selectedRoles = this.data['rolename'].split(',');
      patchData.rolename = this.selectedRoles;
      this.detailFormControls.get('rolename')?.setValue(this.selectedRoles);
    }
...
*/
  ...
  getData() {
    if (environment.prototype && this.id) {
      ...
      this.gcipRoleMappingService.getProtoTypingDataById(params).subscribe((res: any) => {
      ...
      const dataSubscription = this.gcipRoleMappingService.getById(params).subscribe((res: any) => {
        this.data = res || {};
        if (this.data['rolename']) {
          this.data['rolename'] = this.data['rolename'].split(',');
          this.selectedRoles = this.data['rolename'];
          this.detailFormControls.get('rolename')?.setValue(this.selectedRoles);
        }
        this.detailFormControls.get('systemCreated')?.setValue(this.data['systemCreated'].toString());
  ...
  onInit() {
    ...
    this.appBaseService.getUserRoles().subscribe((response: any) => {
      this.rolesArray = Object.entries(response).map(([key, value]) => ({
        name: value,
        value: key
      }));
      if (this.data['rolename'] && (typeof this.data['rolename'] != 'string')) {
        this.selectedRoles = this.data['rolename'];
        this.detailFormControls.get('rolename')?.setValue(this.selectedRoles);
      } else if (this.data['rolename'] && (typeof this.data['rolename'] == 'string')) {
        this.selectedRoles = this.data['rolename'].split(',');
        this.detailFormControls.get('rolename')?.setValue(this.selectedRoles);
      }
    ...
    this.getId();
    this.detailFormControls.get('systemCreated')?.disable();
    if (!!this.id) {
      this.detailFormControls.get('domainname')?.disable();
    } else {
      this.detailFormControls.get('systemCreated')?.setValue('false');
    }
    this.initForm();
