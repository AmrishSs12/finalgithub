...
@Directive({})
export class GoogleCloudIdentityBaseComponent {
    ...
    let uiConfig: any = {
      ...
      callbacks: {
        signInSuccessWithAuthResult: async (authResult: any, redirectUrl: any) => {
          ...
          this.appGlobalService.write('tenantId', null);
  ...
  addSignInOptions(uiConfig: any, config: any) {
    if (config?.providersInfo && config?.providersInfo.length > 0) {
      for (let provider of config.providersInfo) {
        let formattedProvider: any;
        if (provider === 'passwordBased' && config?.loginMethod?.emailSubAuthenticationType === 'passwordBased') {
        ...
        else if (config?.loginMethod?.federatedSubAuthenticationType?.includes(provider)) {
          if (provider === 'google') {
            formattedProvider = {
  ...
  onInit() {
    ...
    this.providersInfo = this.config?.providersInfo;
    if (this.config) {
      this.app = initializeApp(this.config.firebaseConfig);
      this.appGlobalService.write('firebaseapp', this.app);
      this.ui = firebaseui.auth.AuthUI.getInstance() || new firebaseui.auth.AuthUI(getAuth(this.app));
